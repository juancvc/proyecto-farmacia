...
import proyect.core.entity.asistencial.maestras.TgemaeAsis05;
import proyect.core.entity.general_.Tgemae01;
import proyect.core.repository.DAOException;
import proyect.core.repository.interfaces.asistencial.maestra.MaestraAsis05DAO;



 
@Transactional
@Repository("maestraAsis05DAO")
public class MaestraAsis05DAOImp implements MaestraAsis05DAO {
...
public TablaBean getTablaBean(Object object){
	...
	public boolean insertar(TablaBean TablaBean) throws DAOException {   
		Object idMaestraAsis05DAO= null; 
		...
		try {
			...
			idMaestraAsis05DAO = spq.getOutputParameterValue(1);
			if (idMaestraAsis05DAO != null) {
				TablaBean.setCodigo(idMaestraAsis05DAO.toString());
				sw=true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public TablaBean getBuscarPorObjecto(TablaBean TablaBean) throws DAOException {
		...
		List<TgemaeAsis05> lsttabla = null;
	...
	public List<TablaBean> getBuscarPorFiltros(TablaBean tablaBean) throws DAOException { 
		...
		List<TablaBean> lstTablaBean = null;
			...
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis05.buscarPorFiltros");
	...
	private List<TablaBean> deListatablaAListaTablaBean(List<TgemaeAsis05> lstHorarioExcepcion) {
		...
		if (lstHorarioExcepcion != null && lstHorarioExcepcion.size() > 0) {
			...
			for (int i = 0; i < lstHorarioExcepcion.size(); i++) { 
				TgemaeAsis05 entity = lstHorarioExcepcion.get(i);
	...
	private TablaBean detablaATablaBean(TgemaeAsis05 entity) {
		...
		if (entity != null) {
			...
			bean.setValor6(entity.getValgen05());
	...
	public List<TablaBean> listarPorCodigoTabla(String codTabla, int tipo) throws DAOException {
		List<TgemaeAsis05> lstMaestra1 = null;	
		List<TablaBean> lstTablaBean = null;
		System.out.println("listarPorCodigoTabla  "+ codTabla);
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis05.listarPorCodigoTabla");  
	...
	public List<TablaBean> listarPorFiltros(TablaBean tabla, int tipo) throws DAOException {
		...
		List<TablaBean> lstTablaBean = null; 
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis05.buscarPorFiltros");  
			spq.setParameter("CODTABLA", tabla.getTabla());   
			spq.setParameter("VALGEN10", tabla.getValor10());   
			spq.setParameter("TIPO", tipo);   
			if (spq.execute()) {
				lstMaestra1 =  spq.getResultList(); 
			} 
