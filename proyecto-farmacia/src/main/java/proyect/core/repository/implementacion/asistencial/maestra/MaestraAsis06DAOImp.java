...
import proyect.core.entity.asistencial.maestras.TgemaeAsis06;
import proyect.core.entity.general_.Tgemae01;
import proyect.core.repository.DAOException;
import proyect.core.repository.interfaces.asistencial.maestra.MaestraAsis06DAO;



 
@Transactional
@Repository("maestraAsis06DAO")
public class MaestraAsis06DAOImp implements MaestraAsis06DAO {
...
public TablaBean getTablaBean(Object object){
	...
	public boolean insertar(TablaBean TablaBean) throws DAOException {   
		Object idMaestraAsis06DAO= null; 
		...
		try {
			...
			idMaestraAsis06DAO = spq.getOutputParameterValue(1);
			if (idMaestraAsis06DAO != null) {
				TablaBean.setCodigo(idMaestraAsis06DAO.toString());
				sw=true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public TablaBean getBuscarPorObjecto(TablaBean TablaBean) throws DAOException {
		...
		List<TgemaeAsis06> lsttabla = null;
	...
	public List<TablaBean> getBuscarPorFiltros(TablaBean tablaBean) throws DAOException { 
		...
		List<TablaBean> lstTablaBean = null;
			...
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis06.buscarPorFiltros");
	...
	private List<TablaBean> deListatablaAListaTablaBean(List<TgemaeAsis06> lstHorarioExcepcion) {
		...
		if (lstHorarioExcepcion != null && lstHorarioExcepcion.size() > 0) {
			...
			for (int i = 0; i < lstHorarioExcepcion.size(); i++) { 
				TgemaeAsis06 entity = lstHorarioExcepcion.get(i);
	...
	private TablaBean detablaATablaBean(TgemaeAsis06 entity) {
	...
	public List<TablaBean> listarPorCodigoTabla(String codTabla, int tipo) throws DAOException {
		List<TgemaeAsis06> lstMaestra1 = null;	
		List<TablaBean> lstTablaBean = null;
		System.out.println("listarPorCodigoTabla  "+ codTabla);
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis06.listarPorCodigoTabla");  
	...
	public List<TablaBean> buscar(TablaBean tabla) throws DAOException {
		List<TgemaeAsis06> lsttabla = null;	
		List<TablaBean> lstTablaBean = null;
		System.out.println("tabla " + tabla.getNombreCorto());
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis06.buscar");
			spq.setParameter("NOMCORTO", tabla.getNombreCorto());
			spq.setParameter("VALGEN04", tabla.getValor4());
			spq.setParameter("TIPO", 1); 
			 if (spq.execute()) {
				lsttabla =  spq.getResultList(); 
			} 
			if (lsttabla != null && lsttabla.size() > 0) {
