...
import proyect.core.entity.asistencial.maestras.TgemaeAsis03;
import proyect.core.entity.general_.Tgemae01;
import proyect.core.repository.DAOException;
import proyect.core.repository.interfaces.asistencial.maestra.MaestraAsis03DAO;



 
@Transactional
@Repository("maestraASIS03DAO")
public class MaestraAsis03DAOImp implements MaestraAsis03DAO {
...
public TablaBean getTablaBean(Object object){
	...
	public boolean insertar(TablaBean TablaBean) throws DAOException {   
		Object idMaestraAsis03DAO= null; 
		...
		try {
			...
			idMaestraAsis03DAO = spq.getOutputParameterValue(1);
			if (idMaestraAsis03DAO != null) {
				TablaBean.setCodigo(idMaestraAsis03DAO.toString());
				sw=true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public TablaBean getBuscarPorObjecto(TablaBean TablaBean) throws DAOException {
		...
		List<TgemaeAsis03> lsttabla = null;
	...
	public List<TablaBean> getBuscarPorFiltros(TablaBean tablaBean) throws DAOException { 
		...
		List<TablaBean> lstTablaBean = null;
			...
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis03.buscarPorFiltros");
	...
	private List<TablaBean> deListatablaAListaTablaBean(List<TgemaeAsis03> lstHorarioExcepcion) {
		...
		if (lstHorarioExcepcion != null && lstHorarioExcepcion.size() > 0) {
			...
			for (int i = 0; i < lstHorarioExcepcion.size(); i++) { 
				TgemaeAsis03 entity = lstHorarioExcepcion.get(i);
	...
	private TablaBean detablaATablaBean(TgemaeAsis03 entity) {
	...
	public List<TablaBean> listarPorCodigoTabla(String codTabla, int tipo) throws DAOException {
		List<TgemaeAsis03> lstMaestra1 = null;	
		List<TablaBean> lstTablaBean = null;
		System.out.println("listarPorCodigoTabla  "+ codTabla);
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeAsis03.listarPorCodigoTabla");  
