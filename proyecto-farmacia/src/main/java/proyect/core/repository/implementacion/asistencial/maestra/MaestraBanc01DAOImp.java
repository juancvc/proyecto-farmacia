...
import proyect.core.entity.asistencial.maestras.TgemaeBanc01;
import proyect.core.repository.DAOException;
import proyect.core.repository.interfaces.asistencial.maestra.MaestraBanc01DAO;



 
@Transactional
@Repository("MaestraBanc01DAO")
public class MaestraBanc01DAOImp implements MaestraBanc01DAO {
...
public TablaBean getTablaBean(Object object){
	...
	public TablaBean getBuscarPorObjecto(TablaBean TablaBean) throws DAOException {
		...
		List<TgemaeBanc01> lsttabla = null;
	...
	private List<TablaBean> deListatablaAListaTablaBean(List<TgemaeBanc01> lstHorarioExcepcion) {
		...
		if (lstHorarioExcepcion != null && lstHorarioExcepcion.size() > 0) {
			...
			for (int i = 0; i < lstHorarioExcepcion.size(); i++) { 
				TgemaeBanc01 entity = lstHorarioExcepcion.get(i);
	...
	private TablaBean detablaATablaBean(TgemaeBanc01 entity) {
	...
	public List<TablaBean> listarPorCodigoTabla(String codTabla, int tipo) throws DAOException {
		List<TgemaeBanc01> lstMaestra1 = null;	
		List<TablaBean> lstTablaBean = null;
		System.out.println("listarPorCodigoTabla banco  "+ codTabla);
			StoredProcedureQuery spq = em.createNamedStoredProcedureQuery("tgemaeBanc01.listarPorCodigoTabla");  
